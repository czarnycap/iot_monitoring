...
curl -i -XPOST "${INFLUXDB_URL}/query" --data-urlencode "q=CREATE DATABASE ${DATABAS
...
CURRENT_RX=$(echo "${VNSTAT_ONELINE}" | awk -F';' '{print $10}' | sed 's/MiB//' | tr
CURRENT_TX=$(echo "${VNSTAT_ONELINE}" | awk -F';' '{print $12}' | sed 's/kbit\/s//'
HOURLY_RX=$(echo "${VNSTAT_ONELINE}" | awk -F';' '{print $5}' | sed 's/MiB//' | tr -
HOURLY_TX=$(echo "${VNSTAT_ONELINE}" | awk -F';' '{print $14}' | sed 's/MiB\/s//' |
